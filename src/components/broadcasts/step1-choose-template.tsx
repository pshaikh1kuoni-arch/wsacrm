...
export function Step1ChooseTemplate({ selectedTemplate, onSelect, onNext, onBack }: Step1Props) {
  ...
  return (
    <div className="space-y-6">
      ...
      {templates.length === 0 ? (
        <div className="flex h-48 flex-col items-center justify-center rounded-xl bg-card-2">
          ...
          {templates.map((template) => {
            ...
            return (
              <button
                ...
                className={`flex flex-col gap-3 rounded-xl border border-transparent p-4 text-left transition-all ${
                  isSelected
                    ? 'border-primary bg-primary/5 ring-1 ring-primary/30'
                    : 'bg-card-2 hover:bg-muted'
                }`}
              >
